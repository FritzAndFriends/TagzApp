...
- Antiforgery token pattern: ALL provider config UIs must include `<AntiforgeryToken />` immediately after `<EditForm>`. Without it, native HTTP POSTs during prerender fail validation. The `_isInteractive` guard on Submit is a complementary defense — it prevents clicks before SignalR connects, so submissions always go through SignalR (bypassing HTTP POST antiforgery checks). Both together = belt and suspenders.
