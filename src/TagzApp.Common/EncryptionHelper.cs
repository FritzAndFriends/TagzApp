...
public class EncryptionHelper
{
	...
	public string Decrypt(string cipherText)
	{
		...
		try
		{
			if (JsonSerializer.Deserialize<object>(cipherText) is not null)
			{
				// The value is not encrypted
				_Logger?.LogError("The configuration value is not encrypted with encryption enabled, returning as plain text");
				return cipherText;
			}
		}
		catch (JsonException)
		{
			// Not valid JSON, proceed with decryption
		}
